---
title: "Oil Price Surge and Economic Outlook 英语精读笔记"
type: study-note
topic: "2002-passage3-oil-price"
tags:
  - english-reading
  - intensive-reading
  - study-note
  - exam-prep
  - oil-price
difficulty: intermediate
created: 2026-06-16
updated: 2026-06-16
sources:
  - "2002阅读/passage_3.md"
  - "intermediate/2002-passage3-oil-price/formatted-article.md"
  - "intermediate/2002-passage3-oil-price/translation.md"
  - "intermediate/2002-passage3-oil-price/grammar-notes.md"
---

# Oil Price Surge and Economic Outlook 精读笔记

---

## 背景

本文选自 2002 年考研英语阅读理解 Part A，探讨石油价格上涨对经济的影响。文章从 OPEC 减产导致油价飙升切入，回顾 1973 年和 1979—1980 年两次石油危机的教训，论证本轮油价上涨不会像过去那样引发严重经济后果。

**选文理由**：文章结构清晰——先设问（为何没有悲观警告？），再从多个维度（税收占比、经济结构变化、宏观背景）逐层论证，是典型的"现象—分析—结论"型议论文。涉及分数/百分数表达、复合宾语、非限制性定语从句等考研高频考点。

**文章来源**：2002 年全国硕士研究生入学统一考试英语试题，阅读理解 Section II Text 3

**难度**：中等偏上，需掌握倍数表达、分数语法、主谓分隔等核心语法点

---

## 文章原文

⑴Could the bad old days of economic decline be about to return? ⑵Since OPEC agreed to supply-cuts in March, the price of crude oil has jumped to almost $26 a barrel, up from less than $10 last December. ⑶This near tripling of oil prices calls up scary memories of the 1973 oil shock, when prices quadrupled, and 1979-80, when they also almost **tripled**.

> [!abstract]- 长难句分析
> **原句**：This near tripling of oil prices calls up scary memories of the 1973 oil shock, when prices quadrupled, and 1979-80, when they also almost **tripled**.
>
> **主干提取**：
> - **S**（主语）：This near tripling of oil prices
> - **V**（谓语）：calls up
> - **O**（宾语）：scary memories of the 1973 oil shock and 1979-80
>
> **修饰成分**：
>
> | 类型 | 引导词 | 修饰对象 |
> |------|--------|----------|
> | 定语从句（非限制性） | when | the 1973 oil shock |
> | 定语从句（非限制性） | when | 1979-80 |
> | 介词短语 | of | oil prices（修饰 tripling，作"油价的…飙升"） |
> | 名词作定语 | oil | shock（构成 oil shock"石油危机"） |
>
> **结构图解**：
> ```
> 主句: [This near tripling of oil prices] + [calls up] + [scary memories]
>   ├── 介短: (of oil prices) → 修饰 tripling（"油价的…飙升"）
>   ├── 介短: (of ...) → 修饰 memories
>   │     ├── 并列宾语 1: the 1973 oil shock
>   │     │     └── 定从: (when prices quadrupled) → 修饰 shock
>   │     └── 并列宾语 2: 1979-80
>   │           └── 定从: (when they also almost tripled) → 修饰 1979-80
> ```
>
> **参考译文**：油价近乎三倍的飙升唤起了人们对 1973 年石油危机（当时油价翻了两番）和 1979—1980 年（当时油价也几乎涨了三倍）的可怕回忆。
>
> **考点提示**：
> 1. **并列结构识别**：`and` 连接两个并列宾语 `the 1973 oil shock` 和 `1979-80`，各自带一个 `when` 引导的非限制性定语从句。理解时需将 `memories of A, when..., and B, when...` 视为 `(memories of A, when...) and (memories of B, when...)` 的并列。
> 2. **非限制性定语从句**：`when` 引导的从句对先行词起补充说明作用，删除后主句结构仍完整。逗号是关键标志，不可与时间状语从句混淆。
> 3. **倍数动词辨析**：`triple`（增至三倍）、`quadruple`（增至四倍）— 考研常考点。注意 `tripling of oil prices` 中的 `tripling` 是名词化的动名词（"三倍化"），充当主语中心词。
> 4. **词义辨析**：`call up` = 唤起（回忆/情绪），不可望文生义为"打电话"。

⑷Both previous shocks resulted in **double-digit** inflation and global economic decline. ⑸So where are the headlines warning of gloom and doom this time?

> [!abstract]- 长难句分析
> **原句**：So where are the headlines warning of gloom and doom this time?
>
> **主干提取**：
> - **S**（主语，倒装前）：the headlines
> - **V**（谓语，系动词）：are
> - **C**（表语，倒装前）：where
>
> **修饰成分**：
>
> | 类型 | 引导词/形式 | 修饰对象 |
> |------|------------|----------|
> | 现在分词短语 | warning of... | headlines（后置定语） |
> | 介词短语 | of | gloom and doom（修饰 warning） |
> | 副词 | So | 整句（承接上文推论） |
> | 名词短语 | this time | 整句（时间状语） |
>
> **结构图解**：
> ```
> 主句: [the headlines] + [are] + [where]?（特殊疑问句倒装）
>   ├── 副词 So → 承前推论
>   ├── 非谓语: (warning of gloom and doom) → 后置定语，修饰 headlines
>   │     └── 介短: (of gloom and doom) → 修饰 warning
>   └── 状语: this time → 时间状语
> ```
>
> **参考译文**：那么，这一次（报纸上）警告经济萧条与厄运来临的大标题都跑哪儿去了呢？
>
> **考点提示**：
> 1. **修辞性疑问句（反问语气）**："Where are the headlines...?" 形式上是问句，实质上是修辞性反问，等价于 "There are no such headlines"，表达作者的讽刺与质疑。So + 修辞性问句 = 否定/讽刺含义。
> 2. **主系表倒装结构**：正常语序为 "The headlines are where?"，特殊疑问词 where 前置构成特殊疑问句。表语 where 提前是为了强调"在哪"而非"是不是"。
> 3. **现在分词作后置定语**："warning of gloom and doom" 等价于定语从句 "that/which warn of gloom and doom"，修饰 headlines；功能上等同于一个非限制性定语从句。
> 4. **高频搭配**：headlines（头条新闻/大标题）、gloom and doom（悲观与厄运/萧条）、warning of（警告...）— 考研阅读高频词组。
> 5. **So 承前推论**：句首的 So 不是"所以"连接词，而是承接前文 oil price shock 背景，进一步推出"既然过去石油危机都有头条警告，那这次怎么没有？"，暗含今非昔比之意。

---

⑴The oil price was given another push up this week when Iraq suspended oil exports. ⑵Strengthening economic growth, at the same time as winter **grips** the northern **hemisphere**, could push the price higher still in the short term.

> [!abstract]- 长难句分析
> **原句**：Strengthening economic growth, at the same time as winter grips the northern **hemisphere**, could push the price higher still in the short term.
>
> **主干提取**：
> - **S**（主语）：Strengthening economic growth
> - **V**（谓语）：could push
> - **O**（宾语）：the price
> - **OC**（宾语补足语）：higher
>
> **修饰成分**：
>
> | 类型 | 引导词/形式 | 修饰对象 |
> |------|------------|----------|
> | 状语从句（时间） | as | 整句（at the same time as 强化"与此同时"） |
> | 现在分词 | Strengthening | economic growth（前置定语） |
> | 副词 | still | higher（程度状语，强调"还要更高"） |
> | 介词短语 | in the short term | 整句（时间状语） |
> | 形容词 | northern | hemisphere（前置定语，"北半球的"） |
>
> **结构图解**：
> ```
> 主句: [Strengthening economic growth] + [could push] + [the price] + [higher]
>   ├── 状从(时间): (at the same time as [winter grips the northern hemisphere])
>   │     └── 从句主干: [winter] + [grips] + [the northern hemisphere]
>   │           └── 形容词: (northern) → 修饰 hemisphere
>   ├── 副词: still → 修饰 higher（程度递进）
>   └── 介短: (in the short term) → 时间状语
> ```
>
> **参考译文**：强劲的经济增长，加之正值北半球冬季来临，可能在短期内进一步推高油价。
>
> **考点提示**：
> 1. **主谓分隔结构**：主语 `Strengthening economic growth` 之后插入一个较长的状语从句，再出现谓语 `could push`，形成主谓分隔。识别时需跳过状语成分，把"主语 + 谓语"配对找准。
> 2. **as 引导时间状语从句**：`as` 在此表"正当/随着"，强调两事同时发生。`at the same time as` 是 `as` 的同义强化表达，意为"与此同时"。
> 3. **复合宾语 push + O + OC**：`push the price higher` 中 `higher` 是宾语补足语，意为"把…推得更高"。`push / make / keep / find + O + adj.` 是考研常见复合宾语结构。
> 4. **现在分词作前置定语**：`Strengthening economic growth` = "the economic growth that is strengthening"，强调增长正在加强的动态过程。
> 5. **高频搭配与词汇**：
>    - `grip` (v.) 紧握、紧抓、影响
>    - `hemisphere` (n.) 半球 → `the northern hemisphere` 北半球
>    - `in the short term` 短期内（区别于 `in the long term` 长期内）
>    - `higher still` 更高一些（`still` 置于比较级后，表"还要更…"）

---

⑴Yet there are good reasons to expect the economic consequences now to be less severe than in the 1970s.

> [!abstract]- 长难句分析
> **原句**：Yet there are good reasons to expect the economic consequences now to be less severe than in the 1970s.
>
> **主干提取**：
> - **S**（形式主语）：There
> - **V**（谓语）：are
> - **C**（主语补足语 / 真正主语）：good reasons
> - **真实结构**：There + be + 主语 + 后置定语（不定式短语）
>
> **修饰成分**：
>
> | 类型 | 引导词/形式 | 修饰对象 |
> |------|------------|----------|
> | 副词 | Yet | 整句（承前转折） |
> | 不定式短语 | to expect... | reasons（后置定语） |
> | 副词 | now | to be less severe（时间状语） |
> | 介词短语 | in the 1970s | 整句（比较状语，省略句） |
> | 形容词比较级 | less | severe（构成 less severe） |
>
> **结构图解**：
> ```
> 主句(There-be 存在句): [There] + [are] + [good reasons]
>   ├── 副词: Yet → 承前转折
>   └── 后置定语: (to expect [the economic consequences] [(now) to be less severe than in the 1970s])
>         ├── 复合宾语: expect + O + OC
>         │     ├── O: the economic consequences
>         │     └── OC: (now) to be less severe than in the 1970s
>         │           ├── 副词: now → 时间状语
>         │           └── 介短: (in the 1970s) → 比较状语（than 后的省略结构）
>         └── 形容词比较级: less severe
> ```
>
> **参考译文**：然而，我们有充分的理由预期，如今（油价上涨）带来的经济影响将不会像 20 世纪 70 年代那样严重。
>
> **考点提示**：
> 1. **there be 存在句型 + 不定式后置定语**：`There are good reasons to expect...` 中 `to expect...` 是真正主语 `reasons` 的后置定语，等价于 `reasons for expecting...`。"理由做某事"是英语中"理由 + 不定式"的典型搭配。
> 2. **复合宾语 expect + O + OC**：`expect the consequences (now) to be less severe` 中 `to be less severe` 是宾语补足语。`expect + O + to do` 是考研常考结构，含义为"期望/预期…做某事"，与 `want/believe/consider + O + to do` 用法一致。
> 3. **比较状语从句的省略**：`less severe than in the 1970s` 完整形式为 `less severe than (it was severe) in the 1970s`，省略了主语 + 系动词 + 表语。这是考研阅读和翻译中的高频省略形式，识别时需根据语义补全被省略成分。
> 4. **句首 Yet 的转折功能**：`Yet` 在句首相当于 `However/Nevertheless`，表示与上文 ⑸ 句"为什么没有悲观头条"的转折，引出下文"有充分理由认为…"，预示作者观点由设问转入论证。
> 5. **高频搭配与词汇**：
>    - `reasons to do` 充分理由做某事（= reasons for doing）
>    - `consequences` 后果、影响（常用复数形式）
>    - `severe` 严重的、剧烈的（语义强度高于 serious）
>    - `in the 1970s` 20 世纪 70 年代（注意 s 表示年代，与 `in 1970` 区分）
>    - `less severe than...` 比…不那么严重（less + 原级 = not as ... as）

⑵In most countries the cost of crude oil now accounts for a smaller share of the price of petrol than it did in the 1970s. ⑶In Europe, taxes account for up to four-fifths of the retail price, so even quite big changes in the price of crude have a more muted effect on pump prices than in the past.

> [!abstract]- 长难句分析
> **原句**：In Europe, taxes account for up to four-fifths of the retail price, so even quite big changes in the price of crude have a more muted effect on pump prices than in the past.
>
> **主干提取**：
> - **分句 1**：
>   - **S**（主语）：taxes
>   - **V**（谓语）：account for
>   - **O**（宾语）：up to four-fifths of the retail price
> - **分句 2**（由 so 引导的推论性结果分句）：
>   - **S**（主语）：(even quite big) changes (in the price of crude)
>   - **V**（谓语）：have
>   - **O**（宾语）：a more muted effect (on pump prices)
> - **连词**：so（表推论/结果，相当于 therefore）
>
> **修饰成分**：
>
> | 类型 | 引导词/形式 | 修饰对象 |
> |------|------------|----------|
> | 介词短语 | In Europe | 分句 1 整句（地点状语） |
> | 介词短语 | up to | four-fifths（程度/范围，意为"高达/最多"） |
> | 介词短语 | of the retail price | four-fifths（后置定语） |
> | 副词 | even | quite big changes（让步/强化语气的副词） |
> | 副词 | quite | big（程度副词，修饰形容词） |
> | 形容词 | big | changes（前置定语） |
> | 介词短语 | in the price of crude | changes（后置定语） |
> | 形容词比较级 | more | muted（构成 more muted） |
> | 介词短语 | on pump prices | effect（后置定语） |
> | 介词短语 | in the past | 整句（比较状语，省略句） |
>
> **结构图解**：
> ```
> 复合并列句: [分句 1] + [so] + [分句 2]
>   ├── 分句 1: [taxes] + [account for] + [up to four-fifths of the retail price]
>   │     ├── 介短: (In Europe) → 地点状语
>   │     ├── 介短: (up to ...) → 修饰宾语，表示"高达"
>   │     └── 介短: (of the retail price) → 修饰 four-fifths
>   ├── 连词 so → 表推论/结果
>   └── 分句 2: [(even quite big) changes (in the price of crude)] + [have] + [a more muted effect (on pump prices)]
>         ├── 副词: even → 强化 quite
>         ├── 副词: quite → 修饰 big
>         ├── 形容词: big → 修饰 changes
>         ├── 介短: (in the price of crude) → 后置定语，修饰 changes
>         ├── 形容词比较级: more muted → 修饰 effect
>         ├── 介短: (on pump prices) → 后置定语，修饰 effect
>         └── 介短: (in the past) → 比较状语（than 后的省略结构）
> ```
>
> **参考译文**：在欧洲，税收在零售价格中占到了五分之四之多，因此即使原油价格出现相当大的涨幅，对加油站零售价格的影响也比过去要小。
>
> **考点提示**：
> 1. **并列复合句 + so 表推论**：`so` 在此用作并列连词，引出推论性结果，相当于 `therefore/that is why`。考研阅读中常见 "so 引导推论分句" 的结构，注意 so 后通常是完整的陈述句，而非 that 从句。
> 2. **account for 短语**：`account for` = "占…比例/比重、对…作出解释"，是及物性短语动词。常见结构 `X accounts for Y% of Z` = "X 在 Z 中占 Y%"。
> 3. **up to 短语**：`up to four-fifths` = "高达五分之四/最多五分之四"，强调数量之多。
> 4. **分数表达**：`four-fifths` 是分数，分子大于 1 时分母须用复数形式。
> 5. **even 的强化功能**：`even quite big changes` 中 `even` 强化 `quite`，表达让步/退一步的含义。
> 6. **have an effect on 搭配**：`have an effect on...` = "对…产生影响"，`muted` = "缓和的、不明显的"。
> 7. **高频搭配与词汇**：
>    - `retail price` 零售价（与 `wholesale price` 批发价相对）
>    - `crude (oil)` 原油
>    - `pump prices` 加油站零售价
>    - `in the past` 在过去

---

⑴Rich economies are also less dependent on oil than they were, and so less sensitive to **swings** in the oil price. ⑵Energy conservation, a shift to other fuels and a decline in the importance of heavy, energy-intensive industries have reduced oil consumption.

> [!abstract]- 长难句分析
> **原句**：Energy conservation, a shift to other fuels and a decline in the importance of heavy, energy-intensive industries have reduced oil consumption.
>
> **主干提取**：
> - **S**（并列主语，由逗号 + and 连接三部分）：
>   - ① Energy conservation
>   - ② a shift to other fuels
>   - ③ a decline in the importance of heavy, energy-intensive industries
> - **V**（谓语，复合谓语）：have reduced
> - **O**（宾语）：oil consumption
>
> **修饰成分**：
>
> | 类型 | 引导词/形式 | 修饰对象 |
> |------|------------|----------|
> | 介词短语 | to other fuels | a shift（后置定语） |
> | 介词短语 | in the importance of... | a decline（后置定语） |
> | 介词短语 | of heavy, energy-intensive industries | the importance（后置定语） |
> | 复合形容词 | energy-intensive | industries（前置定语） |
> | 形容词 | heavy | industries（前置定语） |
>
> **结构图解**：
> ```
> 主句(长主语+短谓语): [S1, S2 and S3] + [have reduced] + [oil consumption]
>   ├── S1: Energy conservation（不可数名词短语）
>   ├── 并列连词 and → 连接三个并列主语
>   ├── S2: a shift to other fuels
>   │     └── 介短: (to other fuels) → 后置定语，修饰 shift
>   ├── 并列连词 and → 连接三个并列主语
>   └── S3: a decline in the importance of heavy, energy-intensive industries
>         └── 介短: (in the importance of heavy, energy-intensive industries) → 后置定语
>               └── 介短: (of heavy, energy-intensive industries) → 修饰 importance
>                     ├── 形容词: heavy → 修饰 industries
>                     └── 复合形容词: energy-intensive → 修饰 industries
> ```
>
> **参考译文**：能源节约、燃料结构的转变以及重工业等能源密集型产业重要性的下降，已经减少了石油的消耗量。
>
> **考点提示**：
> 1. **长主语句型识别**：本句是典型的"长主语 + 短谓语"结构。识别时**先找谓语动词**（have reduced），再向前回溯找出主语范围。
> 2. **主谓一致**：`and` 连接三个并列主语时，谓语动词用**复数形式** `have`。**陷阱提示**：不可因最近主语是单数 `a decline` 而误用 `has`。
> 3. **复合形容词 `energy-intensive`**：`名词 + 形容词` 构成的复合形容词用连字符连接，作前置定语。
> 4. **介词短语层层嵌套**：识别时需理清每个介词的修饰对象：`in` → decline；`of` → importance；heavy / energy-intensive → industries。
> 5. **高频搭配与词汇**：
>    - `energy conservation` 能源节约
>    - `shift to...` 转向…
>    - `a decline in...` 在…方面的下降
>    - `energy-intensive` (adj.) 能源密集型的
>    - `consumption` (n.) 消费、消耗

⑶Software, consultancy and mobile telephones use far less oil than steel or car production. ⑷For each dollar of GDP (in constant prices) rich economies now use nearly 50% less oil than in 1973. ⑸The OECD estimates in its latest Economic Outlook that, if oil prices averaged $22 a barrel for a full year, compared with $13 in 1998, this would increase the oil import bill in rich economies by only 0.25-0.5% of GDP. ⑹That is less than one-quarter of the income loss in 1974 or 1980. ⑺On the other hand, oil-importing emerging economies—to which heavy industry has shifted—have become more energy-intensive, and so could be more seriously squeezed.

> [!abstract]- 长难句分析
> **原句**：On the other hand, oil-importing emerging economies—to which heavy industry has shifted—have become more energy-intensive, and so could be more seriously squeezed.
>
> **主干提取**：
> - **S**（主语）：oil-importing emerging economies
> - **V1 + C1**（谓语 1 + 表语）：have become more energy-intensive
> - **V2 + C2**（谓语 2 + 表语，由 and so 引出推论）：could be more seriously squeezed
> - **连词**：and so（and 连接复合谓语，so = therefore）
>
> **修饰成分**：
>
> | 类型 | 引导词/形式 | 修饰对象 |
> |------|------------|----------|
> | 介词短语（状语） | On the other hand | 整句（对比/转折） |
> | 复合形容词 | oil-importing | economies（前置定语） |
> | 现在分词（形容词） | emerging | economies（前置定语） |
> | 定语从句（非限制性，破折号包裹） | to which | economies（介词 + 关系代词） |
> | 形容词比较级 | more | energy-intensive |
> | 副词比较级 | more | seriously |
> | 副词 | seriously | squeezed（程度状语） |
>
> **结构图解**：
> ```
> 主句: [S] + [V1 + C1], + and so + [V2 + C2]
>   ├── S: oil-importing emerging economies
>   │     ├── 复合形容词: (oil-importing) → 修饰 economies
>   │     └── 现在分词: (emerging) → 修饰 economies
>   ├── 句首状语: (On the other hand) → 整句（与上文转折）
>   ├── 破折号插入定从: (to which [heavy industry has shifted])
>   │     └── 从句主干: [heavy industry] + [has shifted] (to which)
>   ├── V1 + C1: [have become] + [more energy-intensive]
>   └── V2 + C2: [could be] + [more seriously squeezed]
>         ├── 副词比较级: more → 修饰 seriously
>         └── 副词: seriously → 修饰 squeezed
> ```
>
> **参考译文**：另一方面，石油进口的新兴经济体——重工业已经转移到这些经济体——已经变得更加能源密集，因此（其经济）可能受到更为严重的挤压。
>
> **考点提示**：
> 1. **破折号包裹的非限制性定语从句**：破折号与逗号功能类似，用于引出非限制性定语从句/插入语，删除后主句结构仍完整。
> 2. **"介词 + 关系代词"前置结构**：`to which` = 介词 to + 关系代词 which，which 指代先行词 economies。
> 3. **复合谓语 (compound predicate)**：`have become... and so could be...` 由 `and` 连接两个谓语，**共享同一主语**。
> 4. **become + adj. 比较级表语**：`have become more energy-intensive` 中 `more energy-intensive` 是形容词比较级作表语。
> 5. **被动语态 could be + V-ed**：`could be more seriously squeezed` 中 `squeezed` 是过去分词作表语，表被动含义。
> 6. **高频搭配与词汇**：
>    - `on the other hand` 另一方面
>    - `emerging economies` 新兴经济体
>    - `oil-importing` 石油进口的
>    - `shift to...` 转移到…
>    - `energy-intensive` 能源密集型的
>    - `squeeze` v./n. 挤压、压缩

---

⑴One more reason not to lose sleep over the rise in oil prices is that, unlike the rises in the 1970s, it has not occurred against the background of general commodity-price inflation and global excess demand.

> [!abstract]- 长难句分析
> **原句**：One more reason not to lose sleep over the rise in oil prices is that, unlike the rises in the 1970s, it has not occurred against the background of general commodity-price inflation and global excess demand.
>
> **主干提取**：
> - **S**（主句主语）：One more reason
> - **V**（主句谓语，系动词）：is
> - **C**（主句表语，that 引导的表语从句）：it has not occurred against the background of general commodity-price inflation and global excess demand
>
> **修饰成分**：
>
> | 类型 | 引导词/形式 | 修饰对象 |
> |------|------------|----------|
> | 不定式短语 | not to lose sleep over... | reason（后置定语） |
> | 介词短语 | over the rise in oil prices | lose sleep |
> | 介词短语 | unlike the rises in the 1970s | 整句（插入状语，让步/对比） |
> | 介词短语 | against the background of... | occurred（状语） |
> | 复合形容词 | commodity-price | inflation（前置定语） |
>
> **结构图解**：
> ```
> 主句(主系表): [One more reason] + [is] + [that 从句]
>   ├── 主语后置定语: (not to lose sleep over the rise in oil prices)
>   │     └── 介短: (over the rise in oil prices) → 修饰 lose sleep
>   │           └── 介短: (in oil prices) → 修饰 rise
>   ├── 连词 that → 引导表语从句
>   └── 表语从句: [it] + [has not occurred] + [against the background of...]
>         ├── 插入状语: (unlike the rises in the 1970s) → 让步/对比
>         └── 介短: (against the background of general commodity-price inflation and global excess demand)
> ```
>
> **参考译文**：没有理由为油价上涨而失眠的另一个原因是：与 20 世纪 70 年代的几次涨价不同，这次涨价并未发生在普遍商品价格通胀和全球需求过剩的大背景下。
>
> **考点提示**：
> 1. **主系表 + 表语从句**：`One more reason is that...` 是典型的主系表结构，that 引导表语从句。
> 2. **不定式作后置定语**：`not to lose sleep over the rise in oil prices` 修饰 `reason`。**reason + to do / not to do** 是考研常见搭配。
> 3. **it 指代主语**：表语从句中的 `it` 指代主句主语 `reason`，避免重复。
> 4. **unlike 引导的对比状语**：`unlike` 是介词，意为"与…不同"，常作状语。
> 5. **occur against the background of...** = "在…背景下发生"，固定搭配。
> 6. **lose sleep over** = "为…失眠/担心"（= worry about）。
> 7. **高频搭配与词汇**：
>    - `reason not to do` 没有理由做某事
>    - `lose sleep over` 为…失眠、担心
>    - `commodity` 商品
>    - `inflation` 通货膨胀
>    - `excess demand` 过剩需求
>    - `global` 全球的

⑵A sizable portion of the world is only just emerging from economic decline. ⑶The Economist's commodity price index is broadly unchanging from a year ago. ⑷In 1973 commodity prices jumped by 70%, and in 1979 by almost 30%.

---

## 翻译对照

⑴Could the bad old days of economic decline be about to return? ⑵Since OPEC agreed to supply-cuts in March, the price of crude oil has jumped to almost $26 a barrel, up from less than $10 last December. ⑶This near tripling of oil prices calls up scary memories of the 1973 oil shock, when prices quadrupled, and 1979-80, when they also almost **tripled**. ⑷Both previous shocks resulted in double-digit inflation and global economic decline. ⑸So where are the headlines warning of gloom and doom this time?

⑴经济衰退的糟糕旧日是否即将卷土重来？⑵自今年三月欧佩克（OPEC）同意减产以来，原油价格已从去年12月的不到10美元飙升至接近每桶26美元。⑶油价近三倍的涨幅唤起了人们对1973年石油危机的可怕记忆——当时油价翻了两番；以及1979-1980年的危机——当时油价也几乎涨了两倍。⑷前两次石油危机都导致了两位数的通货膨胀和全球经济衰退。⑸那么这一次，为何没有报刊头条发出悲观与厄运的警告呢？

> [!note] gloom and doom：固定搭配，指"悲观与厄运"，常用于描述对未来前景的忧虑。

---

⑴The oil price was given another push up this week when Iraq suspended oil exports. ⑵Strengthening economic growth, at the same time as winter grips the northern hemisphere, could push the price higher still in the short term.

⑴本周伊拉克暂停石油出口，再次推动油价上涨。⑵经济增长的加强，加之北半球进入冬季，短期内可能进一步推高油价。

---

⑴Yet there are good reasons to expect the economic consequences now to be less severe than in the 1970s. ⑵In most countries the cost of crude oil now accounts for a smaller share of the price of petrol than it did in the 1970s. ⑶In Europe, taxes account for up to four-fifths of the retail price, so even quite big changes in the price of crude have a more muted effect on pump prices than in the past.

⑴然而我们有充分的理由预期，当前的经济影响不会像1970年代那样严重。⑵在大多数国家，原油成本在汽油价格中所占的份额比1970年代更小。⑶在欧洲，税收在零售价中占比高达五分之四，因此即使原油价格出现较大波动，对加油站零售价的影响也比过去更为温和。

> [!note] pump prices：英美国家口语，指"加油站的零售油价"，源于加油站的油泵（petrol pump）。

---

⑴Rich economies are also less dependent on oil than they were, and so less sensitive to swings in the oil price. ⑵Energy conservation, a shift to other fuels and a decline in the importance of heavy, energy-intensive industries have reduced oil consumption. ⑶Software, consultancy and mobile telephones use far less oil than steel or car production. ⑷For each dollar of GDP (in constant prices) rich economies now use nearly 50% less oil than in 1973. ⑸The OECD estimates in its latest Economic Outlook that, if oil prices averaged $22 a barrel for a full year, compared with $13 in 1998, this would increase the oil import bill in rich economies by only 0.25-0.5% of GDP. ⑹That is less than one-quarter of the income loss in 1974 or 1980. ⑺On the other hand, oil-importing emerging economies—to which heavy industry has shifted—have become more energy-intensive, and so could be more seriously squeezed.

⑴富裕经济体对石油的依赖度也比过去更低，因此对油价波动的敏感度也更低。⑵节能、转向其他燃料以及重工业等高能耗产业重要性的下降，都降低了石油消耗。⑶软件、咨询和移动电话行业对石油的消耗远低于钢铁或汽车生产。⑷按不变价格计算，富裕经济体每创造一美元GDP所消耗的石油比1973年减少近50%。⑸经合组织（OECD）在其最新一期《经济展望》中估计，如果油价全年平均为每桶22美元（1998年为13美元），那么富裕经济体的石油进口账单将仅增加GDP的0.25%-0.5%。⑹这一数字不到1974年或1980年收入损失的四分之一。⑺另一方面，承接了重工业转移的石油进口新兴经济体已变得更高能耗，因此可能受到更为严重的挤压。

> [!note] squeezed：原意"被挤压"，此处引申为"受到严重挤压/压力"，形容经济体在油价上涨背景下面临的困境。

---

⑴One more reason not to lose sleep over the rise in oil prices is that, unlike the rises in the 1970s, it has not occurred against the background of general commodity-price inflation and global excess demand. ⑵A sizable portion of the world is only just emerging from economic decline. ⑶The Economist's commodity price index is broadly unchanging from a year ago. ⑷In 1973 commodity prices jumped by 70%, and in 1979 by almost 30%.

⑴不必为油价上涨而忧心忡忡的另一个原因是，与1970年代不同，本轮上涨并非发生在商品价格普遍上涨和全球需求过旺的背景下。⑵世界上相当一部分地区才刚刚走出经济衰退。⑶《经济学人》商品价格指数与一年前基本持平。⑷而1973年商品价格跃升了70%，1979年也上涨了近30%。

> [!note] lose sleep over：字面"因……睡不着觉"，引申为"为……担忧/焦虑"，是英语中常见的惯用表达。

---

## 长难句分析

> [!note] 本篇文章的长难句分析已内联插入到上方「文章原文」各段落中，共 8 个分析块。展开折叠即可查看主干提取、修饰成分表、结构图解、参考译文和考点提示。
>
> 涉及的核心语法点：
> - 并列结构识别（and 连接并列宾语/主语/谓语）
> - 非限制性定语从句（when / to which）
> - 主系表倒装结构
> - 主谓分隔（长主语 + 短谓语）
> - 复合宾语（push + O + OC / expect + O + to do）
> - There-be 存在句 + 不定式后置定语
> - 破折号插入语（非限制性定语从句）
> - 比较状语从句的省略

---

## 语法要点

# 2002 Passage 3 语法整理 - 分数与百分数表达法

> **本文核心语法点**：分数 / 百分数表达法（普通分数、特殊分数、百分数、增减搭配）
>
> **出现位置**：Para 3(3) / Para 4(6) / Para 5(4)
>
> **本文高频分数 / 百分数速览**：four-fifths、one-quarter、50% less、0.25-0.5%、by 70%、by almost 30%

---

### 一、分数表达法 (Fractions)

英语中表达"几分之几"有两种路径：① **公式组装法**（基数词—序数词）；② **专有单词法**（half、quarter）。两者不可混淆。

#### 1.1 核心公式：普通分数的"组装方式"

> [!tip] 组装公式
> **分子（基数词）— 分母（序数词）**
>
> 例：四分之三 = three-fourths / three-quarters

> [!warning] 复数规则
> - 分子 **> 1** 时，分母**必须加 -s**（分母代表"几等份"，是复数概念）。
> - 分子 **= 1** 时，分母用原形（不加 -s）。

> [!note] 连字符（-）规范
> - 分子和分母之间**通常要加连字符**；
> - 如果分数在句中作名词（后面不接名词），连字符可加可不加；
> - **写作时建议一律加上连字符**，避免扣分。

**普通分数示例表**

| 分数 | 正确写法 | 错误写法 |
|------|----------|----------|
| 1/3 | one-third / a third | ~~one-third~~ ❌ 加 the 才错 |
| 2/3 | **two-thirds** | ~~two-third~~ ❌ 漏 -s |
| 3/5 | **three-fifths** | ~~three-fifth~~ ❌ 漏 -s |

---

#### 1.2 必须死记的特殊分数（高频常考）

> [!warning] 重要例外
> 英语中有些分数有**专属单词**，**不能硬套**"基数词—序数词"公式。

| 分数 | 地道写法（推荐） | 公式硬套（不推荐） |
|------|------------------|--------------------|
| 1/2 | **a half** / one half | ❌ ~~one-second~~ |
| 1/4 | **a quarter** / one quarter | 📝 one-fourth（可用但不地道） |
| 3/4 | **three-quarters** | 📝 three-fourths（可用但不地道） |

> [!tip] 记忆口诀
> - **half（半）**和 **quarter（四分之一）** 是两个"VIP 选手"，有自己的名字，写作时优先用。
> - 分子 > 1 时，quarter 同样要加 **-s**（→ three-quarters, two-quarters）。

### 二、本文实例（实战解析）

> [!example] Para 3(3) - up to four-fifths
> **原句**：In Europe, taxes account for **up to four-fifths** of the retail price...
>
> **解析**：四分之五的"零售价"，**five → fifth，分母加 s**（分子 4 > 1）。

> [!example] Para 4(6) - 百分数区间
> **原句**：...if oil prices averaged $22 a barrel for a full year, compared with $13 in 1998, this would increase the oil import bill in rich economies by only **0.25-0.5%** of GDP.
>
> **解析**：百分数区间，连字符连接两个百分数。

> [!example] Para 4(6) - one-quarter
> **原句**：That is less than **one-quarter** of the income loss in 1974 or 1980.
>
> **解析**：特殊分数，**one quarter 不加 -s**（分子 = 1）。

> [!example] Para 5(4) - 50% less
> **原句**：For each dollar of GDP (in constant prices) rich economies now use **nearly 50% less** oil than in 1973.
>
> **解析**：百分数 + 形容词比较级 less，表示"少用近 50%"。

> [!example] Para 5(4) - jumped by + 百分数
> **原句**：In 1973 commodity prices jumped **by 70%**, and in 1979 **by almost 30%**.
>
> **解析**：**jumped by + 百分数** = 增长了百分之多少；"by"是"增长幅度"的标配介词。

### 三、考点速查清单

> [!tip] 写作时务必检查
> 1. ✅ 分子 > 1，分母是否加了 **-s**？
> 2. ✅ 是否优先使用 **half** / **quarter** 等特殊写法？
> 3. ✅ **percent** 是否没有加 -s？
> 4. ✅ 分子分母之间是否加了连字符？
> 5. ✅ 表达增长/减少时是否使用了正确的介词 **by**？

---

## 生词表

| 词汇 | 词性 | 含义 | 原文例句 |
|------|------|------|----------|
| **double-digit** | adj. | 两位数的 | Both previous shocks resulted in **double-digit** inflation and global economic decline. |
| **grips** | v. | 紧握；影响；（寒冬）笼罩 | Strengthening economic growth, at the same time as winter **grips** the northern hemisphere... |
| **hemisphere** | n. | 半球 | ...at the same time as winter grips the northern **hemisphere**... |
| **swings** | n. | 波动；摆动 | ...less sensitive to **swings** in the oil price. |
| **tripled** | v. | 增至三倍 | ...1979-80, when they also almost **tripled**. |

### 生词练习

**一、选词填空**

从方框中选择合适的词汇填入空白处（每词限用一次）：

> double-digit / grips / hemisphere / swings / tripled

1. The company's profits have ________ over the past three years, reaching a record high.

2. The policy caused ________ unemployment, affecting millions of workers across the country.

3. Climate change is having a profound impact on the Arctic ________, with ice sheets melting at an alarming rate.

4. Political instability can ________ a nation for decades, hindering economic progress.

5. Investors are wary of sudden ________ in the stock market, which can wipe out savings overnight.

6. The population of the city has nearly ________ since 2000, putting enormous pressure on infrastructure.

7. The tropical storm is expected to affect the southern ________ before moving northward.

8. Economic ________—periods of rapid growth followed by sharp recessions—make long-term planning difficult.

> [!abstract]- 答案
> 1. **tripled**（增至三倍）
> 2. **double-digit**（两位数的）
> 3. **hemisphere**（半球）
> 4. **grip**（影响；笼罩）
> 5. **swings**（波动）
> 6. **tripled**（增至三倍）
> 7. **hemisphere**（半球）
> 8. **swings**（波动）

**二、短语翻译**

将下列短语翻译成中文：

1. account for up to four-fifths of...

2. lose sleep over...

3. on the other hand

> [!abstract]- 答案
> 1. **account for up to four-fifths of...** = 占到…的五分之四之多（account for = 占…比例）
> 2. **lose sleep over...** = 为…失眠；为…担忧（字面义：因…睡不着觉）
> 3. **on the other hand** = 另一方面（与 on the one hand 相对，用于引出对比观点）

**三、语境理解**

根据上下文，选择正确的词义：

1. "Strengthening economic growth, at the same time as winter **grips** the northern hemisphere..." 中 **grips** 的含义是：
   - A. 握住（物理动作）
   - B. 紧紧控制；笼罩（比喻义）
   - C. 理解；掌握
   - D. 抓住机会

> [!abstract]- 答案
> 1. **B** — grip 在此用作比喻义，表示冬季"笼罩、控制"北半球，形象地表达寒冬到来时对整个半球的影响。

2. "less sensitive to **swings** in the oil price" 中 **swings** 的含义是：
   - A. 秋千
   - B. 摇摆舞
   - C. 波动；起伏变化
   - D. 轮换；更替

> [!abstract]- 答案
> 2. **C** — swings 在经济语境中指价格的"波动、起伏变化"，oil price swings = 油价波动。swing 作名词可表示"摆动、变化"，常用于 financial swings / mood swings 等搭配。

3. "Both previous shocks resulted in **double-digit** inflation" 中 **double-digit** 的含义是：
   - A. 双倍的
   - B. 两位数的（10-99）
   - C. 双重的
   - D. 数字翻倍的

> [!abstract]- 答案
> 3. **B** — double-digit 意为"两位数的"，即 10% 及以上的通胀率。double-digit inflation / growth / unemployment 是经济类文章的常见表达，强调数值之高。

---

## 心得

> [!tip] 文章结构与论证逻辑
> 本文采用经典的"设问—论证—结论"结构：
> 1. **第1段**：设问引入——油价飙升，为何没有悲观警告？（修辞性疑问句）
> 2. **第2段**：进一步强化背景——伊拉克暂停出口 + 冬季需求推高油价
> 3. **第3-5段**：逐层论证"这次不一样"——税收占比变化、经济结构转型、宏观背景不同
> 4. **第6段**：总结——不需要为油价上涨失眠
>
> 作者立场：**乐观**（optimistic），认为本轮油价上涨的影响有限。

> [!warning] 考研命题陷阱
> - **第51题（主旨细节题）**：B 选项"reduction in supply"对应第⑵句 OPEC supply-cuts，是最直接的原因。A 选项"global inflation"是干扰项——文章明确说这次**没有**通胀背景。
> - **第52题（推断题）**：D 选项"oil taxes rise"——文中说欧洲税收占零售价 4/5，税收占比越大，原油价格波动的影响越小；反之若税收上涨，影响才大。需逆向推理。
> - **第55题（态度题）**：作者用"good reasons to expect""less severe""less than one-quarter"等措辞，表明对经济前景持乐观态度。

> [!question] 反思问题
> 1. 文章第⑸句"So where are the headlines...?"是反问句，如果误读为一般疑问句，会如何影响对全文基调的理解？
> 2. "less severe than in the 1970s"中 than 后省略了什么？如果补全应该是什么？
> 3. 本文中 `account for` 出现了几次？每次的含义是否相同？

---

## 延伸

- 相关主题：石油经济、OPEC 产油政策、全球经济周期
- 后续文章建议：
  - [[2000-passage1-america-精读笔记]] — 同为经济类话题，可对比论证风格
  - [[2002-passage4-physician-assisted-suicide-精读笔记]] — 同年其他篇章，拓展词汇量
- 补充学习资源：
  - 考研英语高频词组：account for, lose sleep over, gloom and doom, on the other hand
  - 语法专题：分数/百分数表达法（本文核心语法点）
  - 延伸阅读：《经济学人》(The Economist) — 本文引用了该杂志的商品价格指数

---

## 思考题

**1.（主旨题）** 本文的中心论点是：
- A. 石油价格飙升将引发新一轮全球经济衰退
- B. 本轮油价上涨对经济的影响将不如 1970 年代严重
- C. OPEC 的减产政策是导致油价上涨的根本原因
- D. 新兴经济体比发达经济体更依赖石油

**2.（细节题）** 根据文章，以下哪项是欧洲零售油价中占比最大的部分？
- A. 原油成本
- B. 税收
- C. 运输费用
- D. 炼油利润

**3.（推断题）** 从文章可以推断，1970 年代的石油危机与本轮油价上涨最大的不同在于：
- A. 油价上涨的幅度不同
- B. 涨价发生的宏观经济背景不同
- C. 受影响的国家数量不同
- D. 石油的使用方式不同

**4.（词义题）** 文中 "muted effect" 最接近的含义是：
- A. 显著的影响
- B. 缓和的、不明显的影响
- C. 积极的影响
- D. 不可预测的影响

**5.（态度题）** 作者对本轮油价上涨的总体态度是：
- A. 悲观的 (gloomy)
- B. 乐观的 (optimistic)
- C. 中立的 (neutral)
- D. 恐惧的 (scared)

---

## 相关笔记

- [[2000-passage1-america-精读笔记]]
- [[2002-passage4-physician-assisted-suicide-精读笔记]]
