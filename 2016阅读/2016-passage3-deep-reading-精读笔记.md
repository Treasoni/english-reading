...
sources:
  - "2016 考研英语一 Text 3"
  - "intermediate/2016-passage3-deep-reading/formatted-article.md"
  - "intermediate/2016-passage3-deep-reading/translation.md"
  - "intermediate/2016-passage3-deep-reading/grammar-notes.md"
  - "intermediate/2016-passage3-deep-reading/固定搭配与词组笔记.md"
---
# Deep Reading 精读笔记
...
```mermaid
graph TD
    A[P1 提出问题：That everyone's too busy these days is a cliché——先认下老话，再用 But 收窄到真问题 There's never any time to read] --> B[P2 驳倒时间管理：通常的时间管理技巧 don't seem sufficient；网上满是 tips on making time to read，但 free up the odd 30 minutes doesn't work——一坐下来思绪的 flywheel keeps spinning，要么已 exhausted 到 challenging book's the last thing you need；引 Tim Parks 点破心 itself is inclined to interruption；结论：深度阅读需要的是 can't be obtained merely by becoming more efficient 的特殊时间]
    B --> C[P3 挖出病根：becoming more efficient 本身就是问题的一部分——把时间看作 resource to be maximised，就会 approach it instrumentally，只承认能 advances progress toward some goal 的时间；Immersive reading 却要你愿意 risk inefficiency goallessness even time-wasting；Eberle 的空瓶子 / 传送带隐喻说明那种 fill every bottle 的压力；定性：No mind-set could be worse]
    C --> D[P4 给出解法：So what does work——scheduling regular times for reading；你本以为会更 fuel the efficiency mind-set，但事实上这类 ritualistic behaviour 帮你 step outside time's flow into soul time；limit distractions 靠纸质书或单一功能 e-readers；随身带书也行得通——providing you dip in often enough，阅读成为 the default state，事务只是暂时 surface，随后 drop back down；末句回环：not making time to read, but just reading, and making time for everything else]
```
