...
description: 维护 .learnings/ 经验库，把过多或反复出现的学习记录、错误日志、铁律失效问题聚类诊断，追溯并修改对应 skill、模板、校验脚本或项目规则；修复并验证后再归档或移除已解决记录；同时检查 .codebuddy/skills 与 .claude/skills 的共享功能同步，防止更新 Codex 后 Claude Code 丢失对应能力。用户提到 learnings 太多、错误反复犯、清理经验库、维护自我学习、压缩错误日志、从错误中修技能、同步 Codex 和 Claude Code 技能时触发。
---
...
```bash
python3 .codebuddy/skills/maintain-learnings/scripts/audit_learnings.py --root .
```
...
- skill 问题：`.codebuddy/skills/<skill>/SKILL.md`
- skill 模板问题：`.codebuddy/skills/<skill>/references/`
- 项目级规则问题：`CODEBUDDY.md`
- CodeBuddy hook 问题：`.codebuddy/hooks/`
- Obsidian 格式问题：优先检查 `.codebuddy/skills/obsidian-markdown/`，再检查具体业务 skill

如果准备同步 Claude Code 语义，必须先 `diff` 比对 `.codebuddy/skills/<skill>/` 与 `.claude/skills/<skill>/`，保留 Claude 专属说明；不要用 Codex 版本覆盖 `.claude/`。
...
- 添加或修改校验脚本，让错误能被自动发现。
- 更新 `CODEBUDDY.md` 中的通用规则，但只用于跨 skill 的铁律。
...
```bash
python3 -c 'from pathlib import Path; p=Path(".codebuddy/skills/<skill>/SKILL.md"); t=p.read_text(encoding="utf-8"); assert t.startswith("---\n") and "\n---" in t[4:]; assert "name:" in t and "description:" in t; print("skill metadata ok")'
```
...
- 不要把只适用于某个 skill 的细节提升到 `CODEBUDDY.md`。
