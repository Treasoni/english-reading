...
description: 读取 intermediate 下所有 grammar-notes.md 与单篇 固定搭配与词组笔记.md，按语法类别（从句、非谓语动词等）去重合并，生成跨篇章的综合语法总结笔记与固定搭配汇总笔记。支持增量更新——新增篇章语法笔记后重新运行自动合并，不重复已有内容。用户提到"语法总结"、"汇总语法"、"语法笔记汇总"时触发。
---
...
- 若某篇把固定搭配内容拆成了单独的 `<topic>/固定搭配与词组笔记.md`，该文件同样是本技能的输入源，**不得遗漏**
...
此技能**无需用户手动提供输入**。技能自动扫描 `intermediate/` 目录下的**两类**源文件：

| 源文件 | 归入 | 汇总输出 |
|--------|------|----------|
| `intermediate/**/grammar-notes.md` | 纯语法结构 | 根目录 `语法总结笔记.md` |
| `intermediate/**/固定搭配与词组笔记.md` | 词义谱系、语义辨析、场景、近义对比 | 根目录 `固定搭配与词组笔记.md` |

**只扫描第一类会漏掉仅存在于单篇搭配笔记中的内容**，因此两类必须同时扫描。
...
find intermediate -name "固定搭配与词组笔记.md" -type f | sort
```
...
记录每个文件的**相对路径**和**最后修改日期**，并标注它属于哪一类。

若两类扫描结果合计为 0 个文件，报告："未找到任何 grammar-notes.md 或固定搭配与词组笔记.md 文件。请先通过 organize-grammar 整理至少一篇语法笔记。"流程结束。

向用户报告找到的源文件清单（文件名 + 修改日期），请求确认继续。

### 步骤 1b：检测已有总结并确定模式

分别检查项目根目录下 `语法总结笔记.md` 与 `固定搭配与词组笔记.md` 是否存在，两者独立判定：

**若不存在 → 全量新建模式**：继续执行步骤 2-8。

**若存在 → 增量更新模式**：

1. 只读该汇总笔记的 YAML frontmatter 提取 `processed_sources` 列表：`Read <汇总笔记> offset=0 limit=60`（或 `sed -n '1,60p'`），不要整读文件
2. 将步骤 1 中**属于该文件那一类**的源文件清单与 `processed_sources` 比对，将文件分为四类：
...
3. 若两类合起来 new + modified 数量为 0，报告："所有源文件均未变动，总结已是最新。"流程结束。
4. 若有变化：向用户报告 X 个未变、Y 个新增、Z 个修改、W 个移除。请求确认后 → **跳转到步骤 9（增量合并）**

**差集核验（必做，不得跳过）**：确定合并范围时先算差集，不要凭印象判断"应该都收录了"：

```bash
# 语法总结笔记.md：源文件清单 − processed_sources，结果必须为空
comm -23 <(find intermediate -name "grammar-notes.md" -type f | sort) \
         <(grep -o 'intermediate/[^"]*grammar-notes.md' 语法总结笔记.md | sort -u)
```

- `语法总结笔记.md` 追踪**全部** grammar-notes.md，**差集必须为空**。差集非空即为历史遗漏，必须在本轮补齐后才能报告完成。
- `固定搭配与词组笔记.md` 只追踪**产生了固定搭配内容**的篇章，差集允许非空；但每个未收录的篇章都必须在报告中点名并说明"该篇为纯语法、无可收搭配"。**不得默认忽略**。
- 同一套差集检查也适用于单篇 `固定搭配与词组笔记.md` 这一类源。
...
固定搭配内容按"考什么"分流，两个根文件都要写，不得只写一个：

| 内容形态 | 去处 | 例子 |
|---------|------|------|
| 句法结构、句型框架、语态变形 | `语法总结笔记.md` | `put sth on hold` 的主动 / 被动 / 系表形态；`Make/Find + A + B` 复合宾语 |
| 词义谱系、语体差异、感情色彩、实战场景、近义辨析 | `固定搭配与词组笔记.md` | `terribly / terrible` 义项谱系；`put on hold` vs `postpone / suspend / call off` |

判断顺序：先看这条内容是"怎么造句"还是"什么意思/什么场合用"。前者进语法文件，后者进搭配文件；两者都有的，两边都写，并用 `[[wiki-link]]` 互相指向。

归类方向必须**二次确认**：写完后再回看一遍本轮每条内容，确认没有把语义辨析塞进语法文件、也没有把句型结构只留在搭配文件。
...
9h. 对 `固定搭配与词组笔记.md` 重复 9a–9g 的相同流程，只是源文件类别换成单篇 `固定搭配与词组笔记.md`。**两个根文件必须在同一轮内都更新，不得只更新其中一个。**

9i. 收尾核验（**报告完成前的最后一道关**）：

- 重跑步骤 1b 的差集命令，确认 `语法总结笔记.md` 的差集为空
- 用 `grep` 确认本轮每篇的新标题都已出现在目标根文件中
- 抽查 2-3 篇本轮源文件，逐条核对每个标题内容是否都已映射到两个根文件之一（词源拆解、术语、实战场景、感情色彩等教学细节不得丢失）
- 确认两个根文件中所有表格前都有空行、标题 `#` 后有空格、无嵌套 callout
- 以上全部通过后，才能向用户报告完成
...
- **不得编造内容**：所有语法规则和例句必须来源于 grammar-notes.md 或单篇 固定搭配与词组笔记.md，不得自行添加
- **两类源文件都要合并**：不得只合并 `grammar-notes.md` 而漏掉单篇 `固定搭配与词组笔记.md`
- **差集核验是完成前提**：`语法总结笔记.md` 的源文件差集必须为空；`固定搭配与词组笔记.md` 的差集必须逐条说明理由
- **不得丢失来源**：每条例句必须标注来源篇章
