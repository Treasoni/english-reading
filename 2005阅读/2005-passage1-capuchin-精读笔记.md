---
title: Fairness in Capuchin Monkeys 英语精读笔记
type: study-note
topic: capuchin
tags:
  - english-reading
  - intensive-reading
  - study-note
  - exam-prep
difficulty: intermediate
created: 2026-07-09
updated: 2026-07-09
sources:
  - 2005年考研英语阅读 Passage 1
---

# Fairness in Capuchin Monkeys 卷尾猴的公平意识 精读笔记

## 背景

本文选自2005年考研英语阅读第一篇，主题为动物行为学领域——**卷尾猴的公平意识**。文章通过 Sarah Brosnan 和 Frans de Waal 发表在《自然》杂志上的实验研究，探讨了猴子是否具有与人类相似的公平感和义愤情绪。

> [!note] 文章特点
> - 科普类议论文，结构清晰：提出现象 → 实验设计 → 实验结果 → 结论与展望
> - 考研命题重点：细节题 + 推断题，命题人倾向考察因果逻辑和对比关系
> - 来源：*Nature*（《自然》杂志），学术类阅读的经典文章来源

---

## 文章原文

# Fairness in Capuchin Monkeys

Everybody loves a **fat pay rise**. Yet pleasure at your own can **vanish** if you learn that a colleague has been given a bigger one. Indeed, if he has a reputation for **slacking**, you might even be **outraged**. Such behaviour is regarded as "all too human", with the underlying assumption that other animals would not be capable of this finely developed sense of **grievance**. But a study by Sarah Brosnan and Frans de Waal of Emory University in Atlanta, Georgia, which has just been published in *Nature*, suggests that it is all too monkey, as well.

> [!abstract]- 长难句分析
> **原句**：Yet pleasure at your own can vanish if you learn that a colleague has been given a bigger one.
>
> **主干提取**：
>
> | 成分 | 内容 |
> |------|------|
> | S (主语) | pleasure at your own |
> | V (谓语) | can vanish |
> | A (状语) | if you learn that a colleague has been given a bigger one |
>
> **修饰成分**：
>
> | 类型 | 引导词 | 修饰对象 | 说明 |
> |------|--------|----------|------|
> | 介词短语 | at | pleasure | 限定主语，"自己的那份（加薪）" |
> | 条件状语从句 | if | 主句 can vanish | 说明喜悦感消失的前提条件 |
> | 宾语从句 | that | learn | 充当 learn 的宾语 |
>
> **结构图解**：
> ```
> 主句: [Yet] pleasure (at your own) can vanish
>   ├── 介短: at your own → 修饰 pleasure（"自己的那份"）
>   └── 状从(条件): if you learn (that a colleague has been given a bigger one)
>         └── 名从(宾语): that a colleague has been given a bigger one → learn 的宾语
> ```
>
> **参考译文**：但如果你得知某个同事加薪更多，你那份加薪的喜悦感就会消失。
>
> **考点提示**：
> > [!tip]- 考研语法考点
> > 1. **条件状语从句嵌套宾语从句**
> > 2. **被动语态**：`has been given`
> > 3. **比较替代**：`a bigger one` 中的 `one`

> [!abstract]- 长难句分析
> **原句**：Such behaviour is regarded as "all too human", with the underlying assumption that other animals would not be capable of this finely developed sense of grievance.
>
> **主干提取**：
>
> | 成分 | 内容 |
> |------|------|
> | S (主语) | Such behaviour |
> | V (谓语) | is regarded |
> | C (主语补足语) | as "all too human" |
> | A (伴随状语) | with the underlying assumption that other animals would not be capable of this finely developed sense of grievance |
>
> **修饰成分**：
>
> | 类型 | 引导词 | 修饰对象 | 说明 |
> |------|--------|----------|------|
> | 介词短语 | as | is regarded | 固定搭配 be regarded as...，引出主语补足语 |
> | 介词短语(伴随) | with | 整个主句 | with + 名词 + 同位语从句，伴随说明 |
> | 同位语从句 | that | assumption | 说明 assumption 的具体内容 |
>
> **结构图解**：
> ```
> 主句: Such behaviour is regarded as "all too human"
>   └── 介短(伴随): with the underlying assumption
>         └── 同位从句: that other animals would not be capable of this finely developed sense of grievance → 修饰 assumption
> ```
>
> **参考译文**：这种行为被认为是"人之常情"，其潜在假设是其他动物不会有这种高度发达的不满意识。
>
> **考点提示**：
> > [!tip]- 考研语法考点
> > 1. **同位语从句 vs 定语从句**
> > 2. **with 伴随结构**
> > 3. **be regarded as 固定搭配**
> > 4. **否定句的识别**

> [!abstract]- 长难句分析
> **原句**：But a study by Sarah Brosnan and Frans de Waal of Emory University in Atlanta, Georgia, which has just been published in *Nature*, suggests that it is all too monkey, as well.
>
> **主干提取**：
>
> | 成分 | 内容 |
> |------|------|
> | S (主语) | a study (by Sarah Brosnan and Frans de Waal of Emory University in Atlanta, Georgia) |
> | V (谓语) | suggests |
> | O (宾语) | that it is all too monkey |
> | A (状语) | as well |
>
> **修饰成分**：
>
> | 类型 | 引导词 | 修饰对象 | 说明 |
> |------|--------|----------|------|
> | 介词短语 | by | study | 说明研究的作者 |
> | 介词短语 | of | de Waal | 限定所属机构 |
> | 介词短语 | in | Emory University | 限定大学所在地 |
> | 非限制性定语从句 | which | study | 补充说明研究发表信息 |
> | 宾语从句 | that | suggests | 充当 suggests 的宾语 |
>
> **结构图解**：
> ```
> 主句: [But] a study ... suggests (that it is all too monkey, as well)
>   ├── 介短(嵌套): by...of...in...
>   ├── 定从(非限): which has just been published in Nature
>   └── 名从(宾语): that it is all too monkey
> ```
>
> **参考译文**：但是，由佐治亚州亚特兰大埃默里大学的 Sarah Brosnan 和 Frans de Waal 进行的一项研究——刚刚发表在《自然》杂志上——表明，这也是"猴之常情"。
>
> **考点提示**：
> > [!tip]- 考研语法考点
> > 1. **非限制性定语从句**
> > 2. **多层介词短语嵌套**
> > 3. **分隔结构**

The researchers studied the behaviour of female brown **capuchin** monkeys. They look cute. They are **good-natured, co-operative** creatures, and they share their food readily. Above all, like their female human **counterparts**, they tend to pay much closer attention to the value of "goods and services" than males.

> [!abstract]- 长难句分析
> **原句**：Above all, like their female human counterparts, they tend to pay much closer attention to the value of "goods and services" than males.
>
> **主干提取**：
>
> | 成分 | 内容 |
> |------|------|
> | S (主语) | they |
> | V (谓语) | tend to pay |
> | O (宾语) | much closer attention |
> | A (方向状语) | to the value of "goods and services" |
> | A (比较状语) | than males |
>
> **修饰成分**：
>
> | 类型 | 引导词/标志 | 修饰对象 | 说明 |
> |------|-------------|----------|------|
> | 连接性状语 | Above all | 整个主句 | 递进关系 |
> | 介词短语(类比) | like | 整个主句 | 与人类女性类比 |
> | 介词短语(方向) | to | attention | 固定搭配 pay attention to |
> | 介词短语(所属) | of | value | 限定归属 |
> | 比较状语(省略) | than | 整个谓语 | 比较级结构 |
>
> **结构图解**：
> ```
> 主句: [Above all,] (like...), they tend to pay much closer attention (to ...) than males
>   ├── 连接性状语: Above all
>   ├── 介短(类比): like their female human counterparts
>   ├── 介短(方向): to the value (of "goods and services")
>   └── 比较状语(省略): than males
> ```
>
> **参考译文**：最重要的是，与人类女性一样，它们往往比雄性更关注"商品和服务"的价值。
>
> **考点提示**：
> > [!tip]- 考研语法考点
> > 1. **比较结构省略**
> > 2. **like vs as 区分**
> > 3. **连接性状语 Above all**

Such characteristics make them perfect candidates for Dr. Brosnan's and Dr. de Waal's study. The researchers spent two years teaching their monkeys to exchange **tokens** for food. Normally, the monkeys were happy enough to exchange pieces of rock for slices of cucumber. However, when two monkeys were placed in separate but **adjoining chambers**, so that each could observe what the other was getting in return for its rock, their behaviour became markedly different.

> [!abstract]- 长难句分析
> **原句**：However, when two monkeys were placed in separate but adjoining chambers, so that each could observe what the other was getting in return for its rock, their behaviour became markedly different.
>
> **主干提取**：
>
> | 成分 | 内容 |
> |------|------|
> | S (主语) | their behaviour |
> | V (谓语) | became |
> | C (主语补足语) | markedly different |
> | A (时间状语从句) | when two monkeys were placed in separate but adjoining chambers, so that each could observe what the other was getting in return for its rock |
>
> **修饰成分**：
>
> | 类型 | 引导词 | 修饰对象 | 说明 |
> |------|--------|----------|------|
> | 连接性状语 | However | 整个主句 | 转折逻辑 |
> | 时间状语从句 | when | 主句 became | 设定时间条件 |
> | 目的状语从句 | so that | when 从句 | 说明安排目的 |
> | 宾语从句 | what | observe | what = the thing that |
> | 介词短语 | in return for | getting | 说明交换方式 |
>
> **结构图解**：
> ```
> 主句: [However,] their behaviour became markedly different
>   └── 状从(时间): when two monkeys were placed...
>         └── 状从(目的): so that each could observe (what...)
>               └── 名从(宾语): what the other was getting
> ```
>
> **参考译文**：然而，当两只猴子被放置在分开但相邻的隔间中，以便每只都能观察到对方用石块换到了什么时，它们的行为变得明显不同。
>
> **考点提示**：
> > [!tip]- 考研语法考点
> > 1. **so that 目的状语从句 vs 结果状语从句**
> > 2. **时间状语从句中的被动语态**
> > 3. **what 引导的名词性从句**

In the world of capuchins, grapes are **luxury goods** (and much preferable to cucumbers). So when one monkey was handed a grape in exchange for her token, the second was **reluctant** to hand hers over for a mere piece of cucumber. And if one received a grape without having to provide her token in exchange at all, the other either tossed her own token at the researcher or out of the chamber, or refused to accept the slice of cucumber. Indeed, the mere presence of a grape in the other chamber (without an actual monkey to eat it) was enough to induce **resentment** in a female capuchin.

> [!abstract]- 长难句分析
> **原句**：So when one monkey was handed a grape in exchange for her token, the second was reluctant to hand hers over for a mere piece of cucumber.
>
> **主干提取**：
>
> | 成分 | 内容 |
> |------|------|
> | S (主语) | the second |
> | V (谓语) | was |
> | C (表语) | reluctant |
> | A (不定式补足语) | to hand hers over for a mere piece of cucumber |
> | A (时间状语从句) | when one monkey was handed a grape in exchange for her token |
>
> **修饰成分**：
>
> | 类型 | 引导词 | 修饰对象 | 说明 |
> |------|--------|----------|------|
> | 连接性状语 | So | 整个主句 | 因果逻辑 |
> | 时间状语从句 | when | 主句 was reluctant | 设定前提条件 |
> | 介词短语 | in exchange for | was handed | 获取方式 |
> | 不定式短语(补足) | to | reluctant | 说明不愿意的行为 |
> | 介词短语 | for | hand over | 交换标的物 |
>
> **结构图解**：
> ```
> 主句: [So] (when...), the second was reluctant (to hand hers over for...)
>   ├── 状从(时间): when one monkey was handed a grape (in exchange for...)
>   └── 不定式短语(补足): to hand hers over (for...)
> ```
>
> **参考译文**：所以当一只猴子用代币换到一颗葡萄时，另一只就不愿意为了区区一片黄瓜而交出她的代币。
>
> **考点提示**：
> > [!tip]- 考研语法考点
> > 1. **被动语态的双宾语结构**
> > 2. **形容词补足语 (to do)**
> > 3. **省略**：`the second` 后省略 `monkey`

> [!abstract]- 长难句分析
> **原句**：And if one received a grape without having to provide her token in exchange at all, the other either tossed her own token at the researcher or out of the chamber, or refused to accept the slice of cucumber.
>
> **主干提取**：
>
> | 成分 | 内容 |
> |------|------|
> | S (主语) | the other |
> | V1 (谓语1) | tossed |
> | O1 (宾语1) | her own token |
> | A1 (地点状语) | at the researcher or out of the chamber (并列) |
> | V2 (谓语2) | refused |
> | O2 (宾语2) | to accept the slice of cucumber |
> | A (条件状语从句) | if one received a grape without having to provide her token in exchange at all |
>
> **修饰成分**：
>
> | 类型 | 引导词 | 修饰对象 | 说明 |
> |------|--------|----------|------|
> | 连接词 | And | 整个主句 | 并列衔接 |
> | 条件状语从句 | if | 主句 | 设定前提 |
> | 介词短语(否定伴随) | without | received | 否定伴随 |
> | 动名词短语 | having to provide...at all | without 的宾语 | 作介词宾语 |
> | 介词短语 | at the researcher | tossed | 方向 |
> | 介词短语 | out of the chamber | tossed | 方向 |
> | 不定式短语(宾语) | to accept | refused | 充当宾语 |
>
> **结构图解**：
> ```
> 主句: [And] (if...), the other either tossed ... or refused ...
>   ├── 状从(条件): if one received a grape (without having to provide her token in exchange at all)
>   └── 并列谓语:
>         ├── [either] tossed her own token (at.../out of...)
>         └── [or] refused (to accept...)
> ```
>
> **参考译文**：如果一只猴子完全不用交出代币就得到了葡萄，另一只要么把自己的代币扔向研究人员或扔出隔间，要么拒绝接受那片黄瓜。
>
> **考点提示**：
> > [!tip]- 考研语法考点
> > 1. **并列谓语 either...or...**
> > 2. **without + 动名词**
> > 3. **at all 强调否定**
> > 4. **条件状语从句嵌套复杂介词结构**

The researchers suggest that capuchin monkeys, like humans, are guided by **social emotions**. In the wild, they are a co-operative, group-living species. Such co-operation is likely to be stable only when each animal feels it is not being cheated. Feelings of **righteous indignation**, it seems, are not the preserve of people alone. Refusing a lesser reward completely makes these feelings abundantly clear to other members of the group. However, whether such a **sense of fairness** evolved independently in capuchins and humans, or whether it **stems** from the common ancestor that the species had 35 million years ago, is, as yet, an unanswered question.

> [!abstract]- 长难句分析
> **原句**：Refusing a lesser reward completely makes these feelings abundantly clear to other members of the group.
>
> **主干提取**：
>
> | 成分 | 内容 |
> |------|------|
> | S (主语) | Refusing a lesser reward |
> | V (谓语) | makes |
> | O (宾语) | these feelings |
> | C (宾语补足语) | abundantly clear |
> | A (方向/对象状语) | to other members of the group |
>
> **修饰成分**：
>
> | 类型 | 标志词 | 修饰对象 | 说明 |
> |------|--------|----------|------|
> | 动名词短语(主语) | Refusing | 整个谓语 | 表抽象行为 |
> | 副词 | completely | makes | 程度 |
> | 副词 | abundantly | clear | 程度 |
> | 介词短语(对象) | to | makes clear | 向谁表达 |
> | 介词短语(所属) | of | members | 限定群体归属 |
>
> **结构图解**：
> ```
> 主句: (Refusing a lesser reward) [completely] makes these feelings [abundantly] clear (to...)
>   ├── 动名词(主语): Refusing a lesser reward
>   ├── 副: completely → 修饰 makes
>   ├── 宾补: [abundantly] clear
>   └── 介短(对象): to other members (of the group)
> ```
>
> **参考译文**：完全拒绝较少的报酬，可以清楚地向群体中的其他成员传达这些感受。
>
> **考点提示**：
> > [!tip]- 考研语法考点
> > 1. **动名词短语作主语**
> > 2. **make + 宾语 + 形容词(宾补)**
> > 3. **副词修饰的层级关系**

> [!abstract]- 长难句分析
> **原句**：However, whether such a sense of fairness evolved independently in capuchins and humans, or whether it stems from the common ancestor that the species had 35 million years ago, is, as yet, an unanswered question.
>
> **主干提取**：
>
> | 成分 | 内容 |
> |------|------|
> | S (主语) | whether such a sense of fairness evolved independently in capuchins and humans, or whether it stems from the common ancestor that the species had 35 million years ago |
> | V (谓语) | is |
> | C (表语) | an unanswered question |
> | A (时间状语) | as yet |
>
> **修饰成分**：
>
> | 类型 | 引导词 | 修饰对象 | 说明 |
> |------|--------|----------|------|
> | 连接性状语 | However | 整个主句 | 转折逻辑 |
> | 主语从句1 | whether | 主句 is | "各自独立进化" |
> | 副词 | independently | evolved | 修饰进化方式 |
> | 介词短语 | in | evolved | 限定进化范围 |
> | 主语从句2 | whether | 主句 is | "源于共同祖先" |
> | 介词短语 | from | stems | 说明起源源头 |
> | 定语从句 | that | ancestor | 修饰 ancestor |
> | 时间状语 | 35 million years ago | had | 修饰从句谓语 |
> | 时间状语 | as yet | is | 固定短语 |
>
> **结构图解**：
> ```
> 主句: [However,] (whether ... or whether ...) is, [as yet], an unanswered question
>   ├── 并列主语从句(S):
>   │     ├── [whether] ... evolved independently (in capuchins and humans)
>   │     └── [or] [whether] it stems (from the common ancestor (that...))
>   └── 时间状语: as yet
> ```
>
> **参考译文**：然而，这种公平意识是在卷尾猴和人类身上各自独立进化而来，还是源于3500万年前这两个物种的共同祖先，目前仍是一个未解之谜。
>
> **考点提示**：
> > [!tip]- 考研语法考点
> > 1. **whether 主语从句**
> > 2. **并列主语从句的识别**
> > 3. **定语从句关系词作宾语**
> > 4. **as yet 固定短语**

## 练习题

21．In the opening paragraph, the author introduces his topic by
在开篇段落中，作者通过什么方式引入主题？
[A] posing a contrast 提出对比
[B] justifying an assumption 证明一个假设
[C] making a comparison 进行比较
[D] explaining a phenomenon 解释一个现象

22．The statement "it is all too monkey" (Last line, Paragraph 1) implies that
"it is all too monkey"（第一段最后一行）这句话暗示什么？
[A] monkeys are also outraged by **slack rivals** 猴子也会对懒惰的对手感到愤慨
[B] resenting unfairness is also monkeys' nature 憎恨不公平也是猴子的天性
[C] monkeys, like humans, tend to be jealous of each other 猴子像人类一样会互相嫉妒
[D] no animals other than monkeys can develop such emotions 除了猴子没有动物能产生这种情感

23．Female capuchin monkeys were chosen for the research most probably because they are
雌性卷尾猴被选为研究对象最可能的原因是它们
[A] more inclined to weigh what they get 更倾向于权衡自己所得到的东西
[B] attentive to researchers' instructions 关注研究者的指令
[C] nice in both appearance and temperament 外表和性情都很讨人喜欢
[D] more generous than their male companions 比雄性同伴更大方

24．Dr. Brosnan and Dr. de Waal have eventually found in their study that the monkeys
Brosnan博士和de Waal博士最终在他们的研究中发现猴子
[A] prefer grapes to cucumbers 更喜欢葡萄而不是黄瓜
[B] can be taught to exchange things 可以被教会交换东西
[C] will not be co-operative if feeling cheated 如果感到受骗就不会合作
[D] are unhappy when separated from others 与他人分开时会不开心

25．What can we infer from the last paragraph?
从最后一段我们可以推断出什么？
[A] Monkeys can be trained to develop social emotions. 猴子可以被训练出社会情感。
[B] Human indignation evolved from an uncertain source. 人类的愤怒感来源尚不确定。
[C] Animals usually show their feelings openly as humans do. 动物通常像人类一样公开表达感受。
[D] Cooperation among monkeys remains stable only in the wild. 猴子间的合作只在野外才能保持稳定。

---

## 翻译对照

# Fairness in Capuchin Monkeys 卷尾猴的公平意识

Everybody loves a **fat pay rise**. Yet pleasure at your own can vanish if you learn that a colleague has been given a bigger one. Indeed, if he has a reputation for slacking, you might even be **outraged**. Such behaviour is regarded as "all too human", with the underlying assumption that other animals would not be capable of this finely developed sense of **grievance**. But a study by Sarah Brosnan and Frans de Waal of Emory University in Atlanta, Georgia, which has just been published in *Nature*, suggests that it is all too monkey, as well.

人人都喜欢大幅加薪。但如果你得知某个同事加薪更多，你的喜悦感就会消失。事实上，如果他还有偷懒的名声，你甚至可能会感到**愤慨**。这种行为被认为是"人之常情"，其潜在假设是其他动物不会有这种高度发达的**不满意识**。但是，由佐治亚州亚特兰大埃默里大学的 Sarah Brosnan 和 Frans de Waal 进行的一项研究——刚刚发表在《自然》杂志上——表明，这也是"猴之常情"。

> [!note] 翻译说明
> "all too human" 直译为"太人类了"，意译为"人之常情"；对应的 "all too monkey" 译为"猴之常情"，形成呼应。

---

The researchers studied the behaviour of female brown **capuchin** monkeys. They look cute. They are **good-natured, co-operative** creatures, and they share their food readily. Above all, like their female human counterparts, they tend to pay much closer attention to the value of "goods and services" than males.

研究人员研究了雌性棕色**卷尾猴**的行为。它们看起来很可爱。它们是**性情温和、善于合作**的动物，并且乐于分享食物。最重要的是，与人类女性一样，它们往往比雄性更关注"商品和服务"的价值。

---

Such characteristics make them perfect candidates for Dr. Brosnan's and Dr. de Waal's study. The researchers spent two years teaching their monkeys to exchange **tokens** for food. Normally, the monkeys were happy enough to exchange pieces of rock for slices of cucumber. However, when two monkeys were placed in separate but adjoining chambers, so that each could observe what the other was getting in return for its rock, their behaviour became markedly different.

这些特征使它们成为 Brosnan 博士和 de Waal 博士研究的理想对象。研究人员花了两年时间教这些猴子用**代币**交换食物。通常情况下，猴子们很乐意用石块换取黄瓜片。然而，当两只猴子被放置在分开但相邻的隔间中，使每只都能观察到对方用石块换到了什么时，它们的行为变得明显不同。

---

In the world of capuchins, grapes are **luxury goods** (and much preferable to cucumbers). So when one monkey was handed a grape in exchange for her token, the second was **reluctant** to hand hers over for a mere piece of cucumber. And if one received a grape without having to provide her token in exchange at all, the other either tossed her own token at the researcher or out of the chamber, or refused to accept the slice of cucumber. Indeed, the mere presence of a grape in the other chamber (without an actual monkey to eat it) was enough to induce **resentment** in a female capuchin.

在卷尾猴的世界里，葡萄是**奢侈品**（比黄瓜受欢迎得多）。所以当一只猴子用代币换到一颗葡萄时，另一只就不**愿意**为了一片黄瓜而交出她的代币。如果一只猴子完全不用交出代币就得到了葡萄，另一只要么把自己的代币扔向研究人员或扔出隔间，要么拒绝接受那片黄瓜。事实上，仅仅在另一个隔间里出现一颗葡萄（即使没有猴子去吃它），就足以引起雌性卷尾猴的**不满**。

> [!note] 翻译说明
> "reluctant" 在否定语境中译为"不愿意"比"不情愿"更符合实验情境——猴子明确地拒绝交易。

---

The researchers suggest that capuchin monkeys, like humans, are guided by **social emotions**. In the wild, they are a co-operative, group-living species. Such co-operation is likely to be stable only when each animal feels it is not being cheated. Feelings of **righteous indignation**, it seems, are not the preserve of people alone. Refusing a lesser reward completely makes these feelings abundantly clear to other members of the group. However, whether such a **sense of fairness** evolved independently in capuchins and humans, or whether it stems from the common ancestor that the species had 35 million years ago, is, as yet, an unanswered question.

研究人员认为，卷尾猴和人类一样，受**社会情感**的引导。在野外，它们是合作性的群居物种。只有当每只动物都感到自己没有受骗时，这种合作才可能保持稳定。**义愤**感似乎并非人类独有。完全拒绝较少的报酬，可以向群体中的其他成员清楚地传达这些感受。然而，这种**公平意识**是在卷尾猴和人类身上各自独立进化而来，还是源于3500万年前这两个物种的共同祖先，目前仍是一个未解之谜。

---

## 长难句分析

> [!note] 提示
> 本篇文章的 9 处长难句分析已内联在"文章原文"各段之后，此处不再重复列出。请参见上方 `## 文章原文` 部分各段对应的 `> [!abstract]-` 分析块。

---

## 语法要点

### 固定搭配：表示"源于"的同义替换

本篇文章末句出现了多个表示"源自/来源于"的动词短语，是考研阅读中高频出现的同义替换考点。

> [!warning] 同义替换必记
> 以下四组短语均表示"源于"，在考研阅读中可互换使用，但词源色彩略有不同：
> - **stem from** — 最学术，强调因果关系
> - **originate from** — 最正式，强调起源出处
> - **spring from** — 最形象，强调突然涌现
> - **come from** — 最基础，通用表达

| 结构 | 词源联想 | 例句 |
|------|----------|------|
| **stem from** | stem 名词"茎"→ 动词"像草木那样从某处长出来" | Whether such a sense of fairness **stems from** the common ancestor... |
| **come from** | 最基础的"来自"表达 | (通用表达，适用于多数语境) |
| **spring from** | spring 名词"泉水/春天"→ 动词"像泉水一样涌出" | The idea **springs from** a deep understanding of human nature. |
| **originate from** | 名词 origin 派生而来，强调起源地/起源时间 | The custom **originates from** an ancient tradition. |

> [!tip] 统一公式
> **A + stem/come/spring/originate from + B**
> - **核心逻辑**：A 是结果，B 是原因
> - **脑内快速画箭头**：B → A（因为有 B，才导致了 A）

---

### 补充要点：考研阅读因果逻辑命题点

> [!warning]- ⚠️ 超级重要：颠倒因果是考研阅读的高频陷阱
> 考研阅读最喜欢在选项里 **颠倒因果**。如果文章说"A 源于 B"，错误选项通常会写成"A 导致了 B"（即把 `B → A` 写成 `A → B`）。
>
> **避坑策略**：
> - 看到 `stem from` / `come from` / `spring from` / `originate from` 这类词
> - 立即判断：谁是因、谁是果？
> - 在原文中标注箭头方向 `B → A`
> - 与选项逐一核对因果关系是否匹配

---

## 生词表

| 词汇 | 词性 | 含义 | 原文例句 |
|------|------|------|----------|
| **adjoining chambers** | n. | 相邻的隔间 | when two monkeys were placed in separate but **adjoining chambers** |
| **capuchin** | n. | 卷尾猴 | The researchers studied the behaviour of female brown **capuchin** monkeys. |
| **come from** | v. | 来自（基础表达） | (通用表达) |
| **co-operative** | adj. | 合作的；乐于合作的 | They are good-natured, **co-operative** creatures. |
| **counterparts** | n. | 对应的人/物 | like their female human **counterparts** |
| **fat pay rise** | n. | 大幅加薪 | Everybody loves a **fat pay rise**. |
| **good-natured** | adj. | 性情温和的 | They are **good-natured**, co-operative creatures. |
| **grievance** | n. | 不满；委屈 | this finely developed sense of **grievance** |
| **luxury goods** | n. | 奢侈品 | grapes are **luxury goods** (and much preferable to cucumbers) |
| **originate from** | v. | 起源于 | (正式用词，强调起源地/起源时间) |
| **outraged** | adj. | 愤慨的；震怒的 | you might even be **outraged** |
| **reluctant** | adj. | 不情愿的；勉强的 | the second was **reluctant** to hand hers over |
| **resentment** | n. | 怨恨；不满 | enough to induce **resentment** in a female capuchin |
| **righteous indignation** | n. | 义愤；正义的愤怒 | Feelings of **righteous indignation**, it seems, are not the preserve of people alone. |
| **sense of fairness** | n. | 公平意识 | whether such a **sense of fairness** evolved independently |
| **slack rivals** | n. | 懒惰的对手 | monkeys are also outraged by **slack rivals** |
| **slacking** | n./v. | 偷懒；懈怠 | if he has a reputation for **slacking** |
| **social emotions** | n. | 社会情感 | capuchin monkeys, like humans, are guided by **social emotions** |
| **spring from** | v. | 源于（形象用法） | (spring 名词"泉水/春天"→ 动词"像泉水一样涌出") |
| **stem from** | v. | 源于（学术用法） | whether it **stems from** the common ancestor that the species had 35 million years ago |
| **tokens** | n. | 代币；象征 | teaching their monkeys to exchange **tokens** for food |
| **vanish** | v. | 消失；消散 | pleasure at your own can **vanish** |

### 生词练习

**一、选词填空**

从方框中选择合适的词汇填入空白处（每词限用一次）：

> vanish / reluctant / counterparts / grievance / tokens / resentment / outraged / stems / good-natured / co-operative

1. The workers were __________ when they learned their salaries had been cut without notice.

2. She felt a deep sense of __________ after being treated unfairly by her boss.

3. The happiness of winning the lottery began to __________ as the tax deductions arrived.

4. Our company is looking for __________ and __________ employees who can work well in teams.

5. The success of the project __________ from years of careful planning and research.

6. In many online games, players collect __________ to unlock special features.

7. The children were __________ to share their toys with the new kid in class.

8. The Chinese __________ of the British Prime Minister is the Premier of the State Council.

> [!abstract]- 答案
> 1. **outraged**（感到愤慨的）
> 2. **grievance**（不满/委屈）
> 3. **vanish**（消失）
> 4. **co-operative**（善于合作的），**good-natured**（性情温和的）
> 5. **stems**（源于）
> 6. **tokens**（代币）
> 7. **reluctant**（不情愿的）
> 8. **counterparts**（对应人物）

**二、短语翻译**

将下列短语翻译成中文：

1. a sense of fairness

2. righteous indignation

3. luxury goods

4. adjoining chambers

5. slack rivals

> [!abstract]- 答案
> 1. **a sense of fairness** = 公平意识
> 2. **righteous indignation** = 义愤；正义的愤怒
> 3. **luxury goods** = 奢侈品
> 4. **adjoining chambers** = 相邻的隔间
> 5. **slack rivals** = 懒惰的对手

**三、语境理解**

根据上下文，选择正确的词义：

1. 原文中 "the mere presence of a grape in the other chamber was enough to induce **resentment** in a female capuchin" 中 **resentment** 的含义是：
   - A. 同意
   - B. 不满（正确）
   - C. 惊喜
   - D. 冷漠

2. 原文中 "whether such a **sense of fairness** evolved independently in capuchins and humans" 中 **sense** 的含义是：
   - A. 感官（视觉/听觉）
   - B. 感觉/意识（正确）
   - C. 道理
   - D. 意义

3. 原文中 "Such behaviour is regarded as 'all too human', with the underlying assumption that other animals would not be capable of this finely developed **sense of grievance**" 中 **grievance** 的含义是：
   - A. 悲伤
   - B. 恐惧
   - C. 不满/委屈（正确）
   - D. 罪恶感

> [!abstract]- 答案
> 1. **B** — resentment 指"不满/怨恨"，由 unfair treatment 所引发，符合实验情境。
> 2. **B** — sense of fairness 是固定短语，意为"公平意识"；sense 在此指"感觉/意识"而非感官功能。
> 3. **C** — grievance 表示"委屈/不满"，与 outraged（愤慨）形成递进关系。

## 心得

> [!tip] 文章论证结构
> 本文采用 **引子（人类现象）→ 类比过渡（猴子也有）→ 实验设计 → 实验结果 → 结论与开放问题** 的经典科普论证结构：
> 1. **引言段（P1）**：从人类加薪心理引出主题，通过"all too human"→"all too monkey"的类比过渡到动物研究
> 2. **实验对象介绍（P2）**：解释为何选择雌性卷尾猴
> 3. **实验过程（P3）**：代币训练 → 对比实验设置
> 4. **实验结果（P4）**：三组对比——公平交换 vs 不公平→拒绝 vs 不劳而获→愤慨
> 5. **结论（P5）**：公平感非人类独有 → 开放性问题——独立进化还是同源？

> [!warning] 考研命题重点
> - **因果逻辑是核心**：第 5 段末句的 whether...or whether... 是主旨题和推断题的高频命题点
> - **类比关系**：开篇的人类加薪与猴子实验的类比关系是 21 题的考点
> - **细节定位**：雌性卷尾猴被选中的原因（P2）、实验中的具体行为（P4）是细节题的典型命题点

> [!question] 反思问题
> - 作者为何从人类"加薪愤慨"开篇，而不是直接介绍猴子实验？
> - 实验中"仅仅出现葡萄就能引起不满"这一细节说明了什么？
> - 文章的最后一句话（whether...or whether...）在结构上起到了什么作用？

---

## 延伸

- **相关话题阅读**：2005 年考研英语阅读三篇文章均涉及社会/动物行为学主题，可与本篇对照阅读
- **拓展知识**：Sarah Brosnan 和 Frans de Waal 是灵长类动物行为学领域的知名学者，其关于"公平感"的研究是演化心理学的重要文献
- **跨笔记链接**：
  - `[[2005-passage2-alphabetism-精读笔记]]`（如已创建）
  - `[[固定搭配与词组笔记]]` — 查看本篇文章涉及的词组汇总

---

## 思考题

1. **主旨题**：What is the main idea of the passage?
   - [A] Monkeys have a highly developed sense of fairness similar to humans.
   - [B] Female capuchin monkeys are more co-operative than males.
   - [C] The evolution of fairness in primates remains unclear.
   - [D] Humans and monkeys share a common ancestor from 35 million years ago.

2. **细节题**：According to the passage, why did the monkeys refuse to accept cucumber?
   - [A] They preferred grapes because they were hungrier.
   - [B] They felt unfairly treated when another monkey got a grape.
   - [C] They were trained to reject certain types of food.
   - [D] They were not hungry at the time of the experiment.

3. **推断题**：What can be inferred from the fact that capuchins share food readily?
   - [A] They are more generous than human beings.
   - [B] They have a natural tendency toward co-operation.
   - [C] They do not understand the value of food.
   - [D] They are easily satisfied with what they have.

4. **词义题**：The word "preserve" in Paragraph 5 is closest in meaning to?
   - [A] conservation
   - [B] exclusive domain
   - [C] protection
   - [D] tradition

5. **结构题**：The last paragraph mainly serves to
   - [A] confirm the conclusion of the study
   - [B] raise further questions beyond the study
   - [C] summarize the experimental process
   - [D] suggest practical applications of the findings

---

## 相关笔记

- `[[固定搭配与词组笔记]]` — 同义替换和固定搭配汇总
- `[[语法总结笔记]]` — 从句、非谓语、比较结构等系统语法参考
